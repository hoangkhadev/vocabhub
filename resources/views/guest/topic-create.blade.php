...
<h3 class="text-green-400 text-xl my-1" id="create-success"></h3>
<form id="form-create-topic" enctype='multipart/form-data'>
    @csrf
    ...
    <div class="flex flex-col gap-2 w-1/2 mb-10">
        ...
        <input type="text" name="name" id="name" placeholder="Enter Topic Name"
            class="bg-transparent border-b-2 outline-none text-white focus:border-amber-400" autofocus>
        <p class="text-red-400 text-sm" id="errName"></p>
    </div>
    <ul id="list-item" class="mb-3 flex flex-col gap-4">
        <div class="bg-secondary rounded-xl item">
            <div class="flex justify-between items-center border-b-2 border-b-gray-800 py-4 px-4">
                ...
                <div
                    class="py-1 px-2 -mx-2 hover:bg-slate-700 text-red-400 hover:text-red-400 rounded-lg cursor-pointer remove-item">
            ...
            <div class="px-4 flex items-center gap-x-5">
                ...
                <label for="image-1"
                    class="border-dashed hover:border-amber-400 duration-200 border-2 px-4 py-2 rounded-md group cursor-pointer image">
                    ...
                    <input type="file" name="image[]" id="image-1" hidden>
                </label>
            </div>
        </div>
    </ul>
    ...
    $(document).ready(function () {
            ...
            const html = `
                <div class="bg-secondary rounded-xl item">
                    <div class="flex justify-between items-center border-b-2 border-b-gray-800 py-4 px-4">
                        ...
                        <div class="py-1 px-2 -mx-2 hover:bg-slate-700 text-red-400 hover:text-red-400 rounded-lg cursor-pointer remove-item">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5"
                                stroke="currentColor" class="w-5 h-5">
                                <path stroke-linecap="round" stroke-linejoin="round"
                                    d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" />
                    ...
                    <div class="px-4 flex items-center gap-x-5">
                        ...
                        <label for="image-${++index}" class="border-dashed border-2 px-4 py-2 rounded-md group cursor-pointer image">
                            <div class="flex flex-col items-center group-hover:text-amber-400">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5"
                                    stroke="currentColor" class="w-5 h-5">
                                    <path stroke-linecap="round" stroke-linejoin="round"
                            ...
                            <input type="file" name="image[]" id="image-${++index}" hidden>
                        </label>
                    </div>
                </div>
            `;
        ...
        function updateItemCount() {
            $('#list-item .item').each(function(index) {
                ...
                $(this).find('.image').attr('for', `image-${index + 1}`);
                $(this).find('.image input[type="file"]').attr('id', `image-${index + 1}`);
            });
        }
        ...
        //Xử lý thêm topic
        $('#form-create-topic').on('submit', function(e) {
            e.preventDefault();
            ...
            $('.item').each(function(index) {
                ...
                const imageFile = $(this).find('input[name="image[]"]')[0].files[0];
                
                // Append data for each item
                if(english) {
                    formData.append(`english[${index}]`, english);
                }

                if(vietnamese) {
                    formData.append(`vietnamese[${index}]`, vietnamese);
                }
            ...
            $.ajax({
                url: "{{route('guest.topic.store')}}", 
                type: 'POST',
                data: formData,
                processData: false,
                contentType: false,
                success: function(response) {
                    $('#create-success').text(response.message);
                    $('#reset').click();
                    $('#name').focus();
                },
                error: function(error) {
                    ...
                    if(status === 422) {
                        const errName = errors?.name?.[0] ? errors?.name?.[0] : "";
                        $('#errName').text(errName);
                    }
